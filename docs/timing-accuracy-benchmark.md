...
| 2026-07-09 | Nothing A024 (SDK 36) | 100ms (shipped default) | 15.07 / 16.01 (sessions 1-3) | 13.56 / 15.55 | **Re-run against the *real* shipped default** (`audioOffsetMs = DEFAULT_AUDIO_OFFSET_MS = 0f`, not the `-30f` the benchmark previously hardcoded - see `MetronomeSettings.kt`'s offset defaults now being `0f`, intentional). Required a companion fix: `startAudioScheduling`'s lookahead gate and `beatZeroCountInNanos`'s guard both used to require a *strictly negative* offset, which would have silently disabled lead-scheduling entirely at `0`; widened to `<= 0f`/`> 0f` respectively. Excess over baseline: **~1.5ms (sessions 1-3: 0.72, 2.45, 1.36)** - inside target, matching (if anything marginally beating) the `-30f`-tested row above. Confirms the gating fix is correct, not just theoretically justified. |
| 2026-07-09 | Nothing A024 (SDK 36) | 0ms (count-in disabled) | 34.22 / 35.56 | 13.08 / 15.57 | Same true-default re-run, count-in disabled - excess ~21ms, smaller than the `-30f`-tested equivalent (~53ms) because the gating fix now lets *steady-state* beats at offset=0 lead-schedule too (they used to, at `-30f`, for the same reason) - beat 0 itself still has no count-in-driven lead in this scenario, so it's not "fixed," just less bad than before this fix existed. |
