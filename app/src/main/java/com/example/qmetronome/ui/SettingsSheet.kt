...
import com.example.qmetronome.midi.UsbMidiConnector
import com.example.qmetronome.visualizers.VisualizerRegistry
...
fun SettingsSheet(onDismiss: () -> Unit, onActivateToy: () -> Unit) {
    ...
    val connectedUsbDeviceName by usbMidi.connectedDeviceName.collectAsState()
            ...
            SettingsSection(title = "Clock") {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        if (connectedUsbDeviceName != null) {
                            OutlinedButton(onClick = { usbMidi.disconnect() }) {
                                Text("Disconnect")
                            }
                        }
                    }
                    usbDevices.forEach { device ->
                        Row(
                            ...
                            OutlinedButton(onClick = { usbMidi.connect(device) }) {
                                Text("Connect")
                            }
                        }
                    }
                    Text(
                        text = "Other apps can also send clock straight to \"qMetronome Clock In\" with no cable.",
