...
/** The animated-GIF counterpart to [TimeSignatureDragScreenshotTest] - see [BpmDragVideoTest]'s
...
class TimeSignatureDragVideoTest {
    ...
    @Test
    fun `record dragging beats-per-bar up, then note value down`() {
        ...
        composeTestRule.onScreenshotRoot().recordRoboVideo(
            ...
            filePath = videoPath("time-signature-drag-scrub"),
            videoOptions = RoboVideoOptions(fps = 10),
        ) {
            dragWithIndicator(beatsNumber, steps = 8, stepOffset = Offset(30f, 0f))
            delay(300)
            dragWithIndicator(unitNumber, steps = 8, stepOffset = Offset(-30f, 0f))
            delay(300)
        }
    }
}
