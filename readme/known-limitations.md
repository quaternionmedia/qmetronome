...
- Per-beat-type MIDI Note/CC output (Settings → MIDI Actions), per-beat overrides across any
  phrase/bar (Settings → Beat Overrides), per-phrase actions (Settings → Phrase Actions), and the
  main screen's manual Trigger action (TAP, once HOLD is latched) are all implemented and
  unit-tested against a fake `MidiReceiver`, but not yet verified against real hardware - see
  `docs/usb-midi-test-plan.md` section 7 and its 7.1/7.2/7.3 subsections.
